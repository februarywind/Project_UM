...
public class MeleeAtackBase : MonoBehaviour
{
    ...
    private void Attack(MeleeData meleeData)
    {
        ...
        // 1. 범위 안에 몬스터들을 확인
        int index = Physics.OverlapSphereNonAlloc(transform.position, meleeData.Range, hits, meleeAtackData.TargetLayer);
        ...
        for (int i = 0; i < index; i++)
        {
            ...
            Vector3 dest = hits[i].transform.position.RemoveOne(RemoveDir.Y);
            ...
            if (targetAngle > meleeData.Angle * 0.5f)
                ...
                continue;
            hits[i].GetComponent<IDamagable>().TakeDamage(stat.AttackPower * meleeData.DamageRatio, EAtackElement.Normal, transform);
        }
    }
